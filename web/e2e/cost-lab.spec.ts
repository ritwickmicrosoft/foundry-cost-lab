...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    window.localStorage.setItem('foundry-cost-lab-guided-estimate-v1', 'complete')
  })
...
test('builds and applies an adaptive guided estimate', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await page.goto('/?guide=1')

  await expect(page.getByRole('heading', { name: 'Build a cost scenario' })).toBeVisible()
  await expect(page.getByLabel('Draft estimate')).toBeVisible()
  await page.getByRole('button', { name: /Customer agent/ }).click()
  await page.getByRole('button', { name: /Production/ }).click()
  await page.getByRole('button', { name: /East US 2/ }).click()
  await page.getByRole('button', { name: /Medium/ }).click()
  await page.getByRole('button', { name: /Dedicated endpoint/ }).click()

  await expect(page.getByText('How much dedicated capacity should we reserve?')).toBeVisible()
  await page.getByRole('button', { name: /Business hours/ }).click()
  await expect(page.getByLabel('Instances')).toHaveValue('2')
  await page.getByLabel('Instances').fill('3')
  await page.getByLabel('VM hourly rate').fill('5')
  await page.getByRole('button', { name: 'Continue' }).click()
  await expect(page.getByText('How should requests use the model portfolio?')).toBeVisible()
  await page.getByRole('button', { name: /Quality focused/ }).click()

  await page.getByRole('button', { name: /Private networking/ }).click()
  await page.getByRole('button', { name: /Production monitoring/ }).click()
  await page.getByRole('button', { name: 'Continue' }).click()
  const considerationNote = '5,000 users in Canada Central. Keep dedicated compute always-on with 4 instances at CAD 6.50/hr. Include content safety and an API gateway. Weekly steering meetings.'
  await page.getByLabel('Additional considerations').fill(considerationNote)
  await page.getByRole('button', { name: 'Review estimate' }).click()
  await expect(page.getByText('The calculator remains unchanged until you apply this draft.')).toBeVisible()
  const considerationReview = page.getByLabel('Applied from your note')
  await expect(considerationReview).toBeVisible()
  await expect(considerationReview.getByText('5,000 monthly users')).toBeVisible()
  await expect(considerationReview.getByText('Region: Canada Central')).toBeVisible()
  await expect(considerationReview.getByText('4 dedicated instances')).toBeVisible()
  await expect(considerationReview.getByText('VM rate: CAD 6.50 / hour')).toBeVisible()
  await expect(considerationReview.getByText('Content safety: include')).toBeVisible()
  await expect(considerationReview.getByText(considerationNote)).toBeVisible()
  await page.getByRole('button', { name: 'Remove API gateway: include' }).click()
  await expect(page.getByText('API gateway: include')).toHaveCount(0)
  await expect(page.getByLabel('Draft estimate')).toContainText('5,000 monthly users')
  await expect(page.getByLabel('Draft estimate')).toContainText('Canada Central')
  await expect(page.getByLabel('Draft estimate')).toContainText('qwen--qwen3.6-27b')
  await expect(page.getByLabel('Draft estimate')).toContainText('gpt-5.4-mini')
  await expect(page.getByLabel('Draft estimate')).toContainText('15% additional calls')
  await expect(page.getByLabel('Draft estimate')).toContainText('Mixed hosting')

  await page.getByRole('button', { name: 'Apply estimate' }).click()

  await expect(page.getByRole('heading', { name: 'Build a cost scenario' })).toHaveCount(0)
  await expect(page.getByLabel('Azure region')).toHaveValue('canadacentral')
  await expect(page.getByLabel('Monthly users')).toHaveValue('5000')
  const managedCompute = page.locator('.cost-line').filter({ hasText: 'qwen--qwen3.6-27b' })
  await expect(managedCompute).toContainText('managed compute')
  await expect(managedCompute).toContainText('$18,980.00')
  await page.getByRole('tab', { name: 'Guardrail' }).click()
  await expect(page.getByRole('switch', { name: 'Toggle Standalone Content Safety API' })).toBeChecked()
  await page.getByRole('tab', { name: 'Platform' }).click()
  await expect(page.getByRole('switch', { name: 'Toggle API Management Basic v2' })).not.toBeChecked()
  await page.getByRole('tab', { name: 'Run' }).click()
  const portfolioEditor = page.getByLabel('Model portfolio editor')
  await expect(portfolioEditor).toContainText('2 routes')
  const readiness = page.getByRole('region', { name: 'Pricing readiness' })
  await expect(readiness).toContainText('2 routed model roles')
  await expect(readiness).toContainText('Reasoning deployment · Input')
  await expect(readiness.getByText('Approval blocked')).toBeVisible()
  const reasoningRoute = portfolioEditor.getByLabel('Reasoning assist route')
  await expect(reasoningRoute).toContainText('gpt-5.4-mini')
  await reasoningRoute.getByLabel('Invocation rate').fill('20')
  await expect(reasoningRoute.getByLabel('Invocation rate')).toHaveValue('20')
  await reasoningRoute.getByLabel('Deployment').selectOption('primary')
  await expect(reasoningRoute).toContainText('qwen--qwen3.6-27b')
  await expect(page.locator('.cost-line').filter({ hasText: 'qwen--qwen3.6-27b' })).toHaveCount(1)
  ...
  await page.getByRole('button', { name: 'Skip to calculator' }).click()
...
test('persists, compares, and restores a named scenario', async ({ page }) => {
  ...
  expect(await scenarioDialog.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true)
  expect(await page.locator('#scenario-description').evaluate((element) => Number.parseFloat(getComputedStyle(element).fontSize))).toBeGreaterThanOrEqual(14)
  ...
  await expect(page.getByText('Buyer lens')).toHaveCount(0)
  await expect(page.getByText('Competitive lens')).toHaveCount(0)
  expect(await page.locator('.comparison-matrix__row > div').first().evaluate((element) => Number.parseFloat(getComputedStyle(element).fontSize))).toBeGreaterThanOrEqual(15)
  await page.getByRole('button', { name: 'Generate summary' }).click()
  await expect(page.getByText('Summary', { exact: true })).toBeVisible()
  await expect(page.getByText('Strengths to consider')).toBeVisible()
  await expect(page.getByText('Questions to resolve')).toBeVisible()
  ...
  expect(JSON.parse(comparisonRequest)).toMatchObject({ buyerLens: 'executive', competitorLens: 'none' })
  expect(await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth)).toBe(false)
