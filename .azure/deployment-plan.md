...
- Dedicated two-to-three scenario comparison with deterministic sales metrics, buyer lenses, competitor discovery lenses, and current per-region recalculation
...
- Buyer and competitor lenses change discovery framing, not the calculated totals. The application never fabricates AWS, Google, or Databricks pricing and never labels a scenario or vendor the winner when evidence or pricing coverage differs.
...
- Scenario comparison update: deployed `2026-08-25` UTC with deterministic two-to-three scenario metrics, current per-region recalculation, buyer and competitor discovery lenses, changed assumptions, cost drivers, and an optional cited customer brief
